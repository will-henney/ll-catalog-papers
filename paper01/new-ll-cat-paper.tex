...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{combo-highpass-n}
  \caption{
...
\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{combo-highpass-far-west}
  \caption{
...
\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{combo-highpass-s}
  \caption{
...
\begin{figure*}
  ...
  \caption{Position of bow shock arcs. The light blue circle symbols
    ...
    filled box label are interproplyd bow shock arcs. Green and
    magenta labels indicate the sources that have outer and inner arc with $ R_c / R_0 < 1.5$,
    respectively, while the red labels designate those where both outer and inner arcs
    meet the above condition. Objects from \citet{Ricci:2008a}
    are also included which are represented as follow: red symbols represent the
...
There is a faint emission arc in front of it that is not clear if is associated
with the stars. There is a west faint emission arc, which is probably
...
band filter but is also visible in the \ha{} filter. This arc looks
more a ionization front than a bowshock. This object
...
\text{098-753 (LP Ori).} This is a reflection nebula around a B star
which could be an emission arc. However, this likely bowshock is very
...
object however, particularly the inner edge.

\textit{138-3024.} Figure shows a T Tauri star with an
arc-like structure. However, this arc is probably a filament. It is seen
most strongly blue and green broad band filters, but it is also seen at very
low contrast in the \ha{} filter. 
...
exhibits some bends and wiggles \citep{Bally:2000a}.
It has a thin straight filament in front of it that appears to
be part of a larger structure. Then the arc-like feature looks
more like the intersection of two filaments.
...
by \citet{Bally:2006a}. The evidence for its emission arc is very weak
due to it being extremely faint and asymmetric. The star shows a faint
protrusion to the southeast in the \ha{} filter, probably indicating a
proplyd tail, although the source has not previously been catalogued as
a proplyd.  
...
\input{table-sort-XX-problematic-groups.tex}
\input{fig-stamps-XX-problematic-groups.tex}
\input{fig-stamps-XX-problematic-groups-continued.tex}
