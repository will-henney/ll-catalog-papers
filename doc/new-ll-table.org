...
1. Finish checking for new objects in the Bally fields
   ...
   - Presumably on external disk
     ...
     - [X] But need to get some more since the list is not complete
       - [2021-07-27 Tue] Now decided that we have enough sources
...
3. [X] Do arc fits for the new objects
...
4. [-] Association with stellar sources
   - [X] Luis found correspondence with stellar sources in different catalogs
   - [-] Incorporate this data into the tables 
5. [ ] Reorganize the tables
   - Would it be better to have a single table of all sources, ordered by RA?
6. Regenerate the figures
   - [X] Find where they come from in the other projects
   - [-] Fix the coordinates of 364-411
     - This is in the Robberto field in the S
     - We need to find out whether there was any special tratement for LL7 and make sure to do the same for this source
